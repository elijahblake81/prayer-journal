// src/components/PrayerCard.jsx
import { useNavigate } from 'react-router-dom'
import { format, parseISO } from 'date-fns'
import { useState } from 'react'
import { useAuth } from '../lib/AuthProvider'
import { incrementPrayedCount } from '../lib/firebase'
import { useToast } from './ToastProvider'
import { publishPrayer, unpublishPrayer } from '../lib/firebase'

export default function PrayerCard({ prayer, onMarkAnswered, onUnmarkAnswered, onDelete }) {
  const navigate = useNavigate()
  const isAnswered = !!prayer?.answered
  const [busy, setBusy] = useState(false)
  const { user } = useAuth()
  const { showToast } = useToast()

  const toDate = (d) => {
    if (!d) return null
    if (typeof d?.toDate === 'function') return d.toDate()
    if (typeof d === 'string') return parseISO(d)
    if (d instanceof Date) return d
    return null
  }

  const createdAt = toDate(prayer?.createdAt || prayer?.date)
  const answeredAt = toDate(prayer?.answered?.date)
  const lastPrayedAt = toDate(prayer?.lastPrayedAt)   // 👈 added

  async function handleDelete() {
    if (!confirm('Delete this prayer?')) return
    if (onDelete) await onDelete(prayer.id)
  }

  async function handleUnmarkClick() {
    if (!onUnmarkAnswered || busy) return
    try {
      setBusy(true)
      await onUnmarkAnswered()
      showToast('Marked as open')
    } finally {
      setBusy(false)
    }
  }

  async function handlePrayedClick() {
    if (!user || busy) return
    try {
      setBusy(true)
      await incrementPrayedCount(user.uid, prayer.id)
      showToast('🙏 Noted — praying with you')
    } catch (e) {
      console.error('Failed to increment prayed count', e)
      showToast('Could not record “Prayed”. Try again.', { duration: 3000 })
    } finally {
      setBusy(false)
    }
  }

  async function handlePublish() {
    if (!user || busy) return
    try {
      setBusy(true)
      await publishPrayer(user.uid, prayer)
      showToast('Shared to Public Prayers')
    } catch (e) {
      console.error('Publish failed', e)
      showToast('Could not publish. Try again.', { duration: 3000 })
    } finally {
      setBusy(false)
    }
  }

  async function handleUnpublish() {
    if (!user || busy) return
    try {
      setBusy(true)
      await unpublishPrayer(user.uid, prayer)
      showToast('Unshared from Public Prayers')
    } catch (e) {
      console.error('Unpublish failed', e)
      showToast('Could not unshare. Try again.', { duration: 3000 })
    } finally {
      setBusy(false)
    }
  }

  const prayedCount = Number(prayer?.prayedCount || 0)

  return (
    <article className={`prayer-card ${isAnswered ? 'answered' : ''}`}>
      <div className="prayer-meta">
        <span className="category">{prayer?.category || 'General'}</span>
        {createdAt && <span className="date">{format(createdAt, 'MMM d, yyyy')}</span>}
        {prayer?.publicId && (
          <span className="tag" title="This prayer is shared publicly">Public</span>
        )}
      </div>

      {/* Content */}
      <p className="prayer-content">{prayer?.content ?? ''}</p>

      {/* Optional scripture */}
      {prayer?.scripture && <p className="scripture">"{prayer.scripture}"</p>}

      {/* Optional tags */}
      {Array.isArray(prayer?.tags) && prayer.tags.length > 0 && (
        <div className="tags">
          {prayer.tags.map(t => (
            <span key={t} className="tag">{t}</span>
          ))}
        </div>
      )}

      {/* Answered block */}
      {isAnswered && (
        <div className="answered-info">
          <span className="answered-badge">✓ Answered</span>
          {answeredAt && <span className="answered-date">{format(answeredAt, 'MMM d, yyyy')}</span>}
          {prayer?.answered?.notes && <p className="answered-notes">{prayer.answered.notes}</p>}
        </div>
      )}

      {/* 👇 New compact meta row: count + last prayed */}
      <div className="prayer-meta-row">
        <span className="prayer-badge">
          Prayed {prayedCount} {prayedCount === 1 ? 'time' : 'times'}
        </span>

        {lastPrayedAt && (
          <span className="date">
            Last prayed {format(lastPrayedAt, 'MMM d, yyyy h:mm a')}
          </span>
        )}
      </div>

      {/* === Clean, compact action bar (buttons only) === */}
      <div className="card-actions">
        <button
          className="btn-action btn-prayed"
          onClick={handlePrayedClick}
          disabled={busy}
          aria-label="Mark prayed"
        >
          🙏 Prayed
        </button>

        {!isAnswered ? (
          <button
            className="btn-action"
            onClick={onMarkAnswered}
            disabled={busy}
          >
            Mark as Answered
          </button>
        ) : (
          <button
            className="btn-action"
            onClick={handleUnmarkClick}
            disabled={busy}
          >
            Mark as Open
          </button>
        )}

        {!prayer?.publicId ? (
          <button className="btn-action" onClick={handlePublish} disabled={busy}>
            Make Public
          </button>
        ) : (
          <button className="btn-action" onClick={handleUnpublish} disabled={busy}>
            Unshare
          </button>
        )}

        {/* Edit */}
        <button
          className="btn-icon"
          onClick={() => navigate(`/prayers/${prayer.id}/edit`)}
          disabled={busy}
          aria-label="Edit"
          title="Edit"
        >
          <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16"
            fill="currentColor" viewBox="0 0 24 24" aria-hidden="true">
            <path d="M3 17.25V21h3.75l11-11-3.75-3.75-11 11zM20.71 7.04a1.003 1.003 0 0 0 0-1.42l-2.34-2.34a1.003 1.003 0 0 0-1.42 0l-1.83 1.83 3.75 3.75 1.84-1.82z" />
          </svg>
        </button>

        {/* Delete */}
        <button
          className="btn-icon danger"
          onClick={handleDelete}
          disabled={busy}
          aria-label="Delete"
          title="Delete"
        >
          <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16"
            viewBox="0 0 24 24" aria-hidden="true" fill="currentColor">
            <path d="M18.3 5.71a1 1 0 0 0-1.41 0L12 10.59 7.11 5.7a1 1 0 1 0-1.41 1.41L10.59 12l-4.9 4.89a1 1 0 1 0 1.41 1.41L12 13.41l4.89 4.9a1 1 0 0 0 1.41-1.41L13.41 12l4.9-4.89a1 1 0 0 0-.01-1.4z" />
          </svg>
        </button>
      </div>
    </article>
  )
}